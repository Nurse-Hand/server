...
- 새 API readiness, worker 실행 상태, storage create/rename/delete, 외부 health, 무로그인 MVP 환자 조회가 모두 성공해야 배포 상태를 기록합니다.
...
10. API와 worker를 같은 digest로 교체하고 각각의 readiness를 기다립니다.
