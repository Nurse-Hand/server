...
이 문서는 GitHub Actions가 검증한 API image 한 개를 가비아 단일 서버에 배포하는 절차를 정의합니다. 실제 SSH 접속, 운영 secret 등록, 방화벽 변경, backup 실행은 저장소 변경과 분리합니다.
...
이 절차는 최초 설치가 아니라 기존 정상 API를 보존하는 반복 배포용입니다. 최초 container와 `.env`는 별도 검증 절차로 준비합니다.
