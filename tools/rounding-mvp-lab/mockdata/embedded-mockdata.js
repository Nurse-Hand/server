window.__ROUNDING_MVP_LAB_DATA__ = {
  ...
  patients: {
    ...
    items: [
      ...
      },
      {
        ...
        safetyFlags: ['보행 시 산소 저하', 'I&O 누락 주의'],
        handoffFocus: ['보행 후 SpO2', '산소 감량 조건', 'I&O 입력 확인'],
      },
    ],
  },
  tasks: {
    ...
    items: [
      ...
      },
      {
        ...
        source: 'ROUNDING_RULE',
        notes: '식사량 절반과 sliding scale follow-up 필요',
      },
      {
      ...
      },
      {
        ...
        source: 'ROUNDING_RULE',
        notes: '2차 라운딩 전 EMR chart 보완 필요',
      },
    ],
  },
  rounding: {
    ...
    sessionTemplate: {
      ...
      apiPaths: {
        ...
        audioAnalyze: '/api/v1/rounding-sessions/{sessionId}/audio-chunks',
        tasksExtract: '/api/v1/task-extraction-jobs',
  ...
  audioManifest: {
    scenarioKey: 'SYNTHETIC_MEDICAL_DAY_SHIFT',
    notes:
      '원본 음성은 커밋하지 않고 로컬 record_data 경로 metadata만 유지한다.',
    ...
    items: [
      {
        ...
        relativePathFromToolDir:
          '../../../../../record_data/1차 라운딩 환자1.m4a',
      },
      {
        audioAssetId: 'audio-r1-p102',
        visitId: 'visit-r1-p102',
  ...
  expectedResults: {
    ...
    visitResults: [
      ...
      },
      {
        ...
        patientId: '018f1da8-6c39-4f1d-8f2f-0f9bc2f58101',
        shortSummary:
          '배액 40mL 확인, 통증 2점으로 감소, 보호자 동행 보행 교육 완료',
        patientScript: [
          {
            speaker: 'NURSE',
      ...
      },
      {
        ...
        patientId: '018f1da8-6c39-4f1d-8f2f-0f9bc2f58103',
        shortSummary:
          '보행 1회 후 SpO2 95%, 산소 감량 여부는 evening shift 재판단',
        patientScript: [
          {
            speaker: 'NURSE',
    ...
    analysisJob: {
      mockResponse: {
        data: {
          jobId: 'analysis-job-20260819',
          status: 'SUCCEEDED',
          processedVisitCount: 6,
          patientMatchReviewCount: 2,
          evidenceCount: 14,
          nextAction: 'Evidence와 Task 후보를 확인하세요.',
        },
        meta: {
          ...
          candidates: [
            {
              ...
              evidenceIds: ['ev-r1-p101-2', 'ev-r2-p101-1'],
            },
            {
              taskId: '018f1da8-6c39-4f1d-8f2f-0f9bc2f59102',
              patientId: '018f1da8-6c39-4f1d-8f2f-0f9bc2f58102',
              title: '13:30 혈당 재측정',
              effectivePriority: 'HIGH',
              evidenceIds: ['ev-r1-p102-2', 'ev-r2-p102-1'],
            },
            {
              taskId: '018f1da8-6c39-4f1d-8f2f-0f9bc2f59103',
              patientId: '018f1da8-6c39-4f1d-8f2f-0f9bc2f58103',
              ...
              evidenceIds: ['ev-r1-p103-1', 'ev-r2-p103-1'],
            },
            {
              taskId: '018f1da8-6c39-4f1d-8f2f-0f9bc2f59104',
              patientId: '018f1da8-6c39-4f1d-8f2f-0f9bc2f58103',
              title: 'I&O 누락분 입력 확인',
              effectivePriority: 'MEDIUM',
              evidenceIds: ['ev-r1-p103-3'],
            },
          ],
          nextAction: 'Precheck 질문을 검토하세요.',
        },
        meta: {
          ...
          items: [
            {
              ...
              severity: 'REQUIRED',
              question:
                'P-7A-101 배액량 40mL 재측정 시각과 drain trend를 인수인계 메모에 반영했나요?',
              evidenceIds: ['ev-r1-p101-2', 'ev-r2-p101-1'],
            },
            {
              itemId: 'precheck-2',
              severity: 'REQUIRED',
              question:
                'P-7A-102 혈당 164 이후 저녁 전 추가 체크 계획이 전달되나요?',
              evidenceIds: ['ev-r1-p102-2', 'ev-r2-p102-1'],
            },
            {
              itemId: 'precheck-3',
              severity: 'CHECK',
              question:
                'P-7A-103 산소 감량 판단 보류 사유와 재평가 조건을 적었나요?',
              evidenceIds: ['ev-r1-p103-2', 'ev-r2-p103-2'],
            },
          ],
          nextAction: 'Draft 생성 전 답변 여부를 확인하세요.',
        },
        meta: {
          ...
          sections: [
            {
              ...
              title: 'Situation',
              content:
                '7A 데이 라운딩 기준 즉시 follow-up이 필요한 환자는 P-7A-101, P-7A-102, P-7A-103입니다.',
              citations: ['ev-r2-p101-1', 'ev-r2-p102-1', 'ev-r2-p103-1'],
            },
            {
              section: 'B',
              title: 'Background',
              content:
                'P-7A-101은 복강경 수술 후 배액 추적 중이고 P-7A-102는 식후 혈당 및 오심 경과를 보는 중이며 P-7A-103은 보행 후 산소포화도 변동을 추적 중입니다.',
              citations: ['ev-r1-p101-2', 'ev-r1-p102-3', 'ev-r1-p103-2'],
            },
            {
              section: 'A',
              title: 'Assessment',
              content:
                'P-7A-101은 배액 40mL로 trend 보고가 필요하고 P-7A-102는 혈당 164 재확인 후 추가 저혈당 증상은 없으며 P-7A-103은 보행 후 SpO2 95로 회복되지만 산소 감량 판단은 보류 상태입니다.',
              citations: ['ev-r2-p101-1', 'ev-r2-p102-1', 'ev-r2-p103-2'],
            },
            {
              section: 'R',
              title: 'Recommendation',
              content:
                'evening shift는 P-7A-101 drain trend, P-7A-102 저녁 전 혈당, P-7A-103 산소 감량 조건과 I&O chart completeness를 우선 확인합니다.',
              citations: ['ev-r1-p101-2', 'ev-r2-p102-1', 'ev-r1-p103-3'],
            },
          ],
          nextAction: '실제 finalize API는 별도 구현 필요',
        },
        meta: {
